...
import {
  ...
  Heading1, Heading2, Heading3, Undo, Redo, Palette, Highlighter,
  AlignLeft, AlignCenter, AlignRight, Table, Minus, Indent, Outdent,
  Quote, Space,
} from "lucide-react";
...
export default function ContractToolbar({ editor }: ContractToolbarProps) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-0.5 p-1.5 border-b bg-muted/30">
      <ToolBtn onClick={() => (editor as any).chain().focus().undo().run()} title="Annuler">
      ...
      <ToolBtn
        onClick={() => {
          const currentIndent = parseInt(editor.getAttributes("paragraph")?.indent || "0", 10);
          editor.chain().focus().updateAttributes("paragraph", { indent: currentIndent + 1 }).run();
        }}
      ...
      {/* Blockquote */}
      <ToolBtn onClick={() => editor.chain().focus().toggleBlockquote().run()} active={editor.isActive("blockquote")} title="Encadré">
      ...
      <ToolBtn
        onClick={() => {
          editor.chain().focus().insertContent('<p style="margin: 16px 0;">&nbsp;</p>').run();
        }}
      ...
      {/* Table */}
      <ToolBtn onClick={() => editor.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()} title="Insérer un tableau">
        <Table className="h-4 w-4" />
      </ToolBtn>
      <ToolBtn onClick={() => editor.chain().focus().setHorizontalRule().run()} title="Trait horizontal">
        ...
        <PopoverContent className="w-auto p-2" align="start">
          ...
          <div className="grid grid-cols-4 gap-1">
            {TEXT_COLORS.map((color) => (
              <button key={color} type="button" className="w-6 h-6 rounded border border-border hover:scale-110 transition-transform" style={{ backgroundColor: color }} onClick={() => editor.chain().focus().setColor(color).run()} />
            ))}
          </div>
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button type="button" variant="ghost" size="icon" className="h-8 w-8" title="Surlignage">
          ...
          <div className="grid grid-cols-4 gap-1">
            {HIGHLIGHT_COLORS.map((color) => (
              <button
                ...
                className={cn("w-6 h-6 rounded border border-border hover:scale-110 transition-transform", color === "transparent" && "bg-[repeating-conic-gradient(#ccc_0%_25%,transparent_0%_50%)] bg-[length:8px_8px]")}
