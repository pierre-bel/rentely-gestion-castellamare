            ...
            (async () => {
              try {
                const { data, error } = await supabase
                  .from("bookings")
                  .select("id, listing_id, checkin_date, checkout_date, checkin_time, checkout_time, nights, guests, total_price, cleaning_fee, notes, status, pricing_breakdown, beach_cabin, igloohome_code")
                  .eq("id", booking.id)
                  .maybeSingle();
                if (error) throw error;
                if (data) {
          ...
          onViewDetails={(booking) => {
            (async () => {
              try {
                const { data, error } = await supabase
                  .from("bookings")
                   .select("id, listing_id, checkin_date, checkout_date, checkin_time, checkout_time, nights, guests, total_price, cleaning_fee, notes, status, pricing_breakdown, access_token, beach_cabin, igloohome_code")
                  .eq("id", booking.id)
                  .maybeSingle();
                if (error) throw error;
                if (data) {
