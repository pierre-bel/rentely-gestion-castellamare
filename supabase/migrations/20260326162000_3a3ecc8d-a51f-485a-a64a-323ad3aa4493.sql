...
CREATE OR REPLACE FUNCTION public.get_cleaning_portal_data(
  p_token TEXT,
  p_month_start DATE,
  p_month_end DATE
)
...
DECLARE
  ...
  v_result JSON;
BEGIN
  SELECT cs.id, cs.name, cs.host_user_id, cs.portal_past_months, cs.portal_future_months
  INTO v_staff
  ...
  SELECT json_build_object(
    ...
    'portal_past_months', v_staff.portal_past_months,
    'portal_future_months', v_staff.portal_future_months,
    'listings', (
          ...
          (
            SELECT COALESCE(json_agg(
              json_build_object(
                ...
                'tenant_name', CASE
                  WHEN b.status = 'owner_blocked' THEN COALESCE(
                    NULLIF(split_part(b.notes, ' | ', 1), ''),
                    'Bloqué'
                  )
                  ELSE COALESCE(
                ...
                END,
                'tenant_phone', CASE
                  WHEN b.status = 'owner_blocked' THEN NULL
                  ELSE (
                    SELECT t.phone
                    FROM tenants t
                    WHERE t.id = (b.pricing_breakdown->>'tenant_id')::uuid
            ...
            WHERE b.listing_id = l.id
              AND b.status IN ('confirmed', 'completed', 'owner_blocked')
